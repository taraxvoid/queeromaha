...
export default async function handler(request) {
  ...
  const rawFormName =
    ...
    dataObj?.["form-name"];
  const formName =
    rawFormName != null ? String(rawFormName).trim().toLowerCase() : "";

  const hasMakerFields =
    dataObj && (dataObj.human_name != null || dataObj.humanName != null);
  const hasContactFields = dataObj && dataObj.message != null;
  const isMaker = formName === "maker" || (hasMakerFields && !hasContactFields);

  if (!isMaker) {
    console.log("submission-created: ignored", {
      formName,
      hasMakerFields,
      hasContactFields,
